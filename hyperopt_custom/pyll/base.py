...
class Apply:
    ...
    def __init__(
        ...
        logging.debug("Apply obj created: name: {}\n pos_args: {}\n named_args: {}\n o_len: {}".format(self.name, self.pos_args, self.named_args, self.o_len))
...
class Literal(Apply):
    ...
    def pprint(self, ofile, lineno=None, indent=0, memo=None):
        ...
        if memo is None:
            memo = {}
        if self in memo:
            out.append(str(lineno[0]) + " " * indent + memo[self])
            print(out[-1], file=ofile)
        else:
            ...
            out.append(str(lineno[0]) + " " * indent + msg)
            print(out[-1], file=ofile)
        lineno[0] += 1
        return out
