...
def GMM1(weights, mus, sigmas, low=None, high=None, q=None, rng=None, size=()):
    ...
    logging.info("Sampling from GMM1")
...
def GMM1_lpdf(samples, weights, mus, sigmas, low=None, high=None, q=None):
    ...
    logging.info("Sampling GMM1_lpdf")
    ...
    logging.info("CDF: {}".format(normal_cdf(high, mus, sigmas)))
...
def adaptive_parzen_normal_orig(mus, prior_weight, prior_mu, prior_sigma):
    ...
    logging.info("Calculating adaptive_parzen_normal_orig...")
    logging.info("Mus: {}".format(mus))
...
def adaptive_parzen_normal(mus, prior_weight, prior_mu, prior_sigma, LF=DEFAULT_LF):
    ...
    logging.info("Calculating adaptive_parzen_normal...")
    # logging.info("Mus: {}".format(mus))
...
def ap_uniform_sampler(obs, prior_weight, low, high, size=(), rng=None):
    logging.info("using ap_uniform_sampler...")
...
def ap_quniform_sampler(obs, prior_weight, low, high, q, size=(), rng=None):
    logging.info("Calculating ap_quniform_sampler...")
...
def ap_normal_sampler(obs, prior_weight, mu, sigma, size=(), rng=None):
    logging.info("using ap_normal_sampler...")
...
def ap_qnormal_sampler(obs, prior_weight, mu, sigma, q, size=(), rng=None):
    logging.info("using ap_qnormal_sampler...")
    weights, mus, sigmas = scope.adaptive_parzen_normal(obs, prior_weight, mu, sigma)
...
def build_posterior(
    ...
    for node in nodes:
        ...
        if node not in memo:
            ...
            if node in obs_memo:
                ...
                logging.info("adaptive_parzen_samplers: {}".format(fn))
                ...
                logging.info("below_llik: {}".format(below_llik))
                logging.info("above_llik: {}".format(above_llik))
    ...
    #logging.info("post_idxs: {}".format(post_idxs))
    #logging.info("post_vals: {}".format(post_vals))
...
def suggest(
    ...
    logging.info("len(docs): {} n_startup_jobs: {}".format(len(docs), n_startup_jobs))
    if len(docs) < n_startup_jobs:
        # N.B. THIS SEEDS THE RNG BASED ON THE new_id
        logging.info("returning rand.suggest...")
    ...
    logging.info("post_ids: {} post_vals: {}".format(idxs, vals))
